...
app = Blueprint("libraries", __name__)
...
def get_Libraries():
    try:
        ...
        data = list(db.Libraries.find({"user_id": user_id, "status": 1}))
...
@app.route("/v1/get_details/library", methods=["POST"])
def get_unique_library():
    try:
        ...
        sub_models_cursor = db.Libraries.find({"_id": {"$in": object_ids}})
        sub_models = []
...
def add_Library():
    try:
        ...
        existing_library = db.Libraries.find_one({"user_id": user_id, "name": name})
        if existing_library:
        ...
        result = db.Libraries.insert_one(data)
...
@app.route("/v1/delete/libraries", methods=["DELETE"])
def delete_model():
    try:
        ...
        models = list(db.Libraries.find({"_id": {"$in": object_ids}}))
        ...
        all_models = list(
            db.Libraries.find({"user_id": user_id, "status": 1}, {"_id": 1}).sort("_id", 1)
        )
